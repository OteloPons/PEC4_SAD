---
title: "PEC 4 - Software para el Análisis de Datos"
...
Definimos entorno de trabajo e instalamos/cargamos bibliotecas necesarias
...
install_package("ggpubr")        # para mostrar resultados de comparación en gráficos
}
...
library("ggpubr")                # para mostrar resultados de comparación en gráficos
```
...
grafico_5 <- ggplot(diabetes_df_limpio, aes (x = diabetes, y = ratio, fill = diabetes)) +
  geom_boxplot() +       # Definimos tipo de gráfico
  xlab("Diabetes") +     # Etiqueta eje X
  ylab ("Ratio HDL/colesterol total") +       # Etiqueta eje Y
  ggtitle ("Comparación ratio HDL/colesterol total en pacientes diabéticos y no diabéticos") +         # Título gráfico
 theme_classic()

# Añadir la comparación de medianas y las estrellas de significación
grafico_estad <- grafico_5 + stat_compare_means(
  method = "wilcox.test",                   # empleamos el test de Wilcoxon-Silk
  comparisons = list(c("NO", "SI")),        # comparamos los 2 grupos
  label = "p.signif"                        # mostramos las estrellas de significación (valor p)
)
...
https://search.r-project.org/CRAN/refmans/ggpubr/html/stat_compare_means.html
