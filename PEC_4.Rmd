...
#diabetes_df <- read.csv("~/4. Máster Bioinformática/Asignaturas/24_25_2_Software para el Análisis de Datos/Tema 4/PEC4_SAD/data/diabetes.csv")
str(diabetes_df)
...
plot_grid(grafico_7, grafico_8,  ncol = 2, nrow =1)
```
...
plot(density(observaciones_muestreadas$glyhb),
     main = "Gráfico de Densidad de tu_variable",
     xlab = "Valores de tu_variable")
```
...
## Regresión múltiple para ver determinación de la diabetes
...
```{r}
estandarizar_datos_diabetic <- scale(columnas_numericas)
head(estandarizar_datos_diabetic)
```

```{r}
  # Aplicamos PCA
pca_resultado <- prcomp(estandarizar_datos_diabetic, center = TRUE, scale =
TRUE)
...
modelo_logistico <- glm(diabetes ~ BMI, data = datos, family = "binomial")

  # Representación gráfica del modelo.
ggplot(data = datos, aes(x = BMI, y = diabetes)) +
  geom_point(aes(color = as.factor(diabetes)), shape = 1) +
  stat_function(fun = function(x){predict(modelo_logistico,
                                          newdata = data.frame(BMI = x),
                                          type = "response")}) +
  theme_bw() +
  labs(title = "Regresión logística",
       y = "Probabilidad diabetes") +
...
# Ajuste de un modelo logístico.
modelo_logistico <- glm(diabetes ~ WHR, data = datos, family = "binomial")

# Representación gráfica del modelo.
ggplot(data = datos, aes(x = WHR, y = diabetes)) +
  geom_point(aes(color = as.factor(diabetes)), shape = 1) +
  stat_function(fun = function(x){predict(modelo_logistico,
                                          newdata = data.frame(WHR = x),
                                          type = "response")}) +
  theme_bw() +
  labs(title = "Regresión logística",
       y = "Probabilidad diabetes") +
...
# Ajuste de un modelo logístico.
modelo_logistico <- glm(diabetes ~ ratio, data = datos, family = "binomial")

# Representación gráfica del modelo.
ggplot(data = datos, aes(x = ratio, y = diabetes)) +
  geom_point(aes(color = as.factor(diabetes)), shape = 1) +
  stat_function(fun = function(x){predict(modelo_logistico,
...
Este gráfico muestra que hay una relación positiva entre el ratio y la probabilidad de tener diabetes:
a mayor ratio, mayor es la probabilidad estimada de diabetes.
