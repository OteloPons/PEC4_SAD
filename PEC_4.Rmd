...
datos_pca <- diabetes_df_limpio %>%
  select(all_of(variables_pca)) %>%
  na.omit()

# Añadir la columna de clase al data frame
datos_pca$diabetes <- na.omit(diabetes)

# Realizar el PCA (sin la variable 'diabetes')
pca_resultado <- prcomp(datos_pca %>% select(-diabetes), scale. = TRUE)
...
autoplot(pca_resultado, data = datos_pca, colour = 'diabetes') +
...
Vemos que ahora tenemos una precisión y una snesibilidad mejores. Para ver si podemos aceptar el modelo, haremos nuevamente una curva ROC.
